// js/matakuliah.js - Menarik dan menampilkan data Kelas Gabungan (Versi Komponen)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (pageTitle) pageTitle.innerText = 'Mata Kuliah';
    ...
    // 3. Logout dengan Event Delegation (Karena tombol ada di komponen yang dimuat dinamis)
    ...
    // 4. Panggil data kelas mahasiswa
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            result.data.forEach(item => {
                ...
                // KARTU MAHASISWA (Sesuai dengan kode Anda yang sudah ada)
                card.innerHTML = `
                    <div>
...
// --- FUNGSI MODAL DETAIL PERTEMUAN (Tidak Diubah) ---
...
async function bukaModalDetail(id_kelas, nama_matkul) {
    ...
    try {
        const response = await fetch(CONFIG.API_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'get_jadwal_pertemuan', id_kelas: id_kelas })
        });
        const result = await response.json();
        ...
        if (result.status === 'success' && result.data.length > 0) {
            ...
            result.data.forEach(pert => {
                ...
                const judulMateri = pert.judul_materi && pert.judul_materi.toString().trim() !== '' ? pert.judul_materi : 'Judul Materi (Belum diisi)';
                let jamTampil = '';
                ...
                html += `
                    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div class="flex flex-col md:flex-row items-start md:items-center gap-2 w-full md:w-auto">
                            ...
                            ${pert.link_materi ? `<a href="${pert.link_materi}" target="_blank" class="w-full md:w-auto bg-teal-50 hover:bg-teal-100 border border-teal-200 text-teal-600 px-4 py-2 rounded-lg text-sm font-bold text-center"><i class="fa-solid fa-file-arrow-down mr-1.5"></i> Materi</a>` : `<span class="w-full md:w-auto text-slate-400 text-sm bg-slate-100 px-4 py-2 rounded-lg text-center border border-slate-200"><i class="fa-regular fa-folder-open mr-1.5"></i> Belum ada materi</span>`}
                        </div>
                    </div>
                `;
            });
